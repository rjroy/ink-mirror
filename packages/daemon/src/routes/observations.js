...
import { observationId, ClassifyObservationRequestSchema, CurationStatusSchema, isValidTransition, } from "@ink-mirror/shared";
 ...
 */
export function createObservationRoutes(deps) {
    const app = new Hono();
    ...
    app.get("/observations/pending", async (c) => {
        ...
        const getEntryText = async (entryId) => {
            const entry = await entryStore.get(entryId);
            return entry?.body;
        };
