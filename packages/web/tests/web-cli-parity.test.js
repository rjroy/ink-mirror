...
describe("web-cli parity", () => {
    ...
    test("POST /entries returns identical entry from GET /entries/:id", async () => {
        const createRes = await hono.request(req("/entries", { method: "POST", body: { body: "Web test entry.", title: "Test Title" } }));
...
`;
        const putRes = await hono.request(req("/profile", { method: "PUT", body: { markdown } }));
    ...
    test("PATCH /observations/:id rejects invalid ID format", async () => {
        const res = await hono.request(req("/observations/not-an-obs-id", { method: "PATCH", body: { status: "intentional" } }));
        expect(res.status).toBe(400);
    });
