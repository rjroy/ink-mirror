...
describe("full loop integration", () => {
    ...
    beforeEach(() => {
        ...
        const onEntryCreated = async (entryId, _entryText) => {
        ...
        const observationRoutes = createObservationRoutes({
            ...
            onIntentional: async (pattern, dimension) => {
                await profileStore.addOrMergeRule(pattern, dimension);
            },
        });
    ...
    test("write -> observe -> curate -> profile", async () => {
        // Step 1: Write entry
        const createRes = await hono.request(req("/entries", {
            method: "POST",
        ...
        const session = await pendingRes.json();
        ...
        const classifyRes = await hono.request(req(`/observations/${obsToClassify.id}`, {
            method: "PATCH",
    ...
    test("observations accessible after entry creation", async () => {
        await hono.request(req("/entries", {
            method: "POST",
    ...
    test("profile is editable after rule creation", async () => {
        ...
        const createRes = await hono.request(req("/entries", { method: "POST", body: { body: "Entry." } }));
        const entry = await createRes.json();
        if (entry.observations && entry.observations.length > 0) {
            await hono.request(req(`/observations/${entry.observations[0].id}`, {
                method: "PATCH",
                body: { status: "intentional" },
            }));
        }
        ...
        if (profile.rules.length > 0) {
            // Edit rule
            const editRes = await hono.request(req(`/profile/rules/${profile.rules[0].id}`, {
                method: "PATCH",
            ...
            // Delete rule
            const deleteRes = await hono.request(req(`/profile/rules/${profile.rules[0].id}`, { method: "DELETE" }));
    ...
    test("EventBus receives observation events during entry creation", async () => {
        ...
        await app.hono.request(req("/entries", { method: "POST", body: { body: "test text for streaming" } }));
