...
export function ProfileEditor({ initialProfile }) {
    ...
    const handleEditRule = useCallback(async (ruleId) => {
        if (editing === ruleId) {
            setSaving(true);
            setError(null);
            try {
    ...
    if (markdownMode) {
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
          ...
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <button onClick={() => setMarkdownMode(false)} style={{ padding: "0.4rem 0.8rem", border: "1px solid #ddd", borderRadius: "4px", cursor: "pointer" }}>
              Cancel
            </button>
            <button onClick={handleSaveMarkdown} disabled={saving} style={{
        ...
        <textarea value={markdownContent} onChange={(e) => setMarkdownContent(e.target.value)} rows={20} style={{
                width: "100%",
                padding: "0.75rem",
                fontSize: "0.9rem",
    ...
    return (<div style={{ maxWidth: "48rem", margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem" }}>
        ...
        <button onClick={() => setMarkdownMode(true)} style={{ padding: "0.4rem 0.8rem", border: "1px solid #ddd", borderRadius: "4px", cursor: "pointer" }}>
          Edit as Markdown
        </button>
      </div>
      ...
      {profile.rules.length === 0 ? (<p style={{ color: "#666" }}>
          No profile rules yet. Write entries and curate observations to build your style profile.
        </p>) : (Object.entries(rulesByDimension).map(([dimension, rules]) => (<div key={dimension} style={{ marginBottom: "1.5rem" }}>
            <h3 style={{ fontSize: "1rem", textTransform: "capitalize", marginBottom: "0.5rem" }}>
              {dimension.replace("-", " ")}
            </h3>
            {rules.map((rule) => (<div key={rule.id} style={{
                    display: "flex",
                ...
                <div style={{ flex: 1 }}>
                  {editing === rule.id ? (<input type="text" value={editValue} onChange={(e) => setEditValue(e.target.value)} style={{
                        width: "100%",
                        padding: "0.3rem",
                        border: "1px solid #ddd",
                        borderRadius: "4px",
                ...
                <button onClick={() => void handleEditRule(rule.id)} disabled={saving} style={{ padding: "0.3rem 0.5rem", fontSize: "0.8rem", border: "1px solid #ddd", borderRadius: "4px", cursor: "pointer" }}>
                  {editing === rule.id ? "Save" : "Edit"}
                </button>
                <button onClick={() => void handleDeleteRule(rule.id)} style={{ padding: "0.3rem 0.5rem", fontSize: "0.8rem", border: "1px solid #ddd", borderRadius: "4px", cursor: "pointer", color: "#c00" }}>
                  Delete
                </button>
              </div>))}
          </div>)))}
